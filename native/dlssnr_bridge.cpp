...
struct Session {
    ...
    bool Initialize(const wchar_t* runtime, const wchar_t* core, uint32_t w,
        uint32_t h, int requestedGpu, std::string& error) {
        ...
        ParamSet(parameters, "DLSSNR.Hint.Render.Preset", 1);
    ...
    bool Process(const uint8_t* input, uint32_t inputStride, const float* depth,
        ...
        context11->UpdateSubresource(input11, 0, nullptr, input, inputStride, 0);
        ...
        for (uint32_t y = 0; y < height; ++y) {
            ...
            if (paperWhite > 0.9999f && paperWhite < 1.0001f) {
                std::memcpy(destination, source, static_cast<size_t>(width) * 4);
                continue;
            }
            for (uint32_t x = 0; x < width; ++x) {
                for (uint32_t channel = 0; channel < 3; ++channel) {
                    const float value = source[x * 4 + channel] / 255.0f;
                    const float adjusted = paperWhite <= 1.0f
                        ? value * paperWhite
...
extern "C" __declspec(dllexport) void* dlssnr_create(
    ...
    uint32_t height, int gpuIndex, char* error, int errorCapacity) {
    auto* session = new Session();
    std::string message;
    if (!session->Initialize(runtimePath, corePath, width, height, gpuIndex, message)) {
